        ...
        threads.forEach(function(thread) {
            ...
            if (thread.recipients.length == 1) {
                ...
                if (recipient.image_url) {
                    ...
                    imageUrl = chrome.runtime.getURL('assets/images/chip_person.png')
                }
            } else {
                name = thread.recipients.map(function(recipient) { return recipient.name }).join(', ')
                imageUrl = chrome.runtime.getURL('assets/images/chip_group.png')
            }
...
var newSmsRow = function() {
    var row = createStreamRow(chrome.runtime.getURL('assets/images/chip_add.png'), chrome.i18n.getMessage('new_sms_thread'))
    row.id = 'new_sms'
    row.onclick = function() {
        selectCompose(row)
    }
